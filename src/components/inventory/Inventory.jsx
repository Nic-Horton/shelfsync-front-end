...
const Inventory = () => {
	...
	const cachedData = queryClient.getQueryData('pantryItems');
	...
	if (inventoryQuery.isLoading && !cachedData) return <h1>Loading...</h1>;
	if (inventoryQuery.isError) {
		return <h1>Error: {inventoryQuery.error.message}</h1>;
	}

	const filteredItems = inventoryQuery.data.filter(
		(item) =>
